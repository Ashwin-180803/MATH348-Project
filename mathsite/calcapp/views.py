...
def compute(request):
    ...
    if mode == "curve":
        try:
            ...
            if not curve:
                return JsonResponse({"ok": False, "error": "Curve type not specified"}, status=400)
            ...
            try:
                ...
                return JsonResponse({"ok": False, "error": f"Error parsing parameter range: {str(e)}"}, status=400)
            ...
            try:
                ...
                error_msg = str(e) if str(e) else f"Error computing curve positions: {type(e).__name__}"
            ...
            if symbolic_quantity:
                ...
                if symbolic_quantity:
                    try:
                        ...
                        symbolic["symbolic_error"] = f"Error getting symbolic formula: {str(e)}"
                        symbolic_quantity = ""  
                            ...
                            if key != "exprs" and key != "var":
                                try:
                                    ...
                                    for sym in param_list[i].free_symbols:
                                        if str(sym) == key:
                                            param_list[i] = param_list[i].xreplace({sym: param_value})
                ...
                try:
                    for i in range(len(param_list)):
                        try:
                            param_list[i] = parser.parse_input(str(param_list[i]), [t_sym])
                        except Exception as parse_err:
                            symbolic["parse_error"] = f"Error parsing expression {i+1}: {str(parse_err)}"
                ...
                if symbolic_quantity == "arc_length":
                    try:
                        ...
                        try:
                            steps = functions.get_computation_steps("arc_length", param_list, [t_sym])
                        ...
                        if isinstance(s_expr, dict):
                            ...
                            symbolic["arc_length_error"] = s_expr.get("msg", "No elementary antiderivative found.")
                            try:
                                symbolic["arc_length_integral"] = sp.latex(s_expr.get("integral", ""))
                            except:
                                symbolic["arc_length_integral"] = str(s_expr.get("integral", ""))
                        else:
                            try:
                                ...
                                symbolic["arc_length_warning"] = f"LaTeX conversion failed: {str(latex_err)}"
                    except Exception as e:
                        symbolic["arc_length_error"] = f"Error computing arc length: {str(e)}"

                elif symbolic_quantity == "reparam_arc_length":
                    try:
                        # Add computation steps
                        try:
                            steps = functions.get_computation_steps("reparam_arc_length", param_list, [t_sym])
                            symbolic["computation_steps"] = steps
                        except Exception:
                            pass  
                        ...
                        if isinstance(rep, dict):
                            ...
                            elif "No elementary antiderivative" in error_msg or "antiderivative" in error_msg.lower():
                            ...
                            if "integral" in rep:
                                try:
                                    symbolic["reparam_integral"] = sp.latex(rep["integral"])
                                    symbolic["reparam_integral_note"] = "The arc length is given by this integral, which cannot be evaluated in closed form:"
                                except:
                            ...
                            if "equation" in rep:
                                try:
                                    symbolic["reparam_equation"] = sp.latex(rep["equation"])
                                except:
                                    
                                    if "equation_str" in rep:
                                        symbolic["reparam_equation"] = rep["equation_str"]
                                    else:
                                        symbolic["reparam_equation"] = str(rep["equation"])
                            ...
                            for i, coord in enumerate(rep):
                                try:
                                    ...
                                    symbolic[f"x_{i + 1}(s)"] = str(coord)
                                    symbolic[f"reparam_warning_{i+1}"] = f"LaTeX conversion failed: {str(latex_err)}"
                    except Exception as e:
                        error_msg = str(e) if str(e) else f"{type(e).__name__}: {repr(e)}"
                        symbolic["reparam_error"] = f"Error computing reparametrization: {error_msg}"

                elif symbolic_quantity == "frenet":
                    try:
                        # Add computation steps
                        try:
                            steps = functions.get_computation_steps("frenet", param_list, [t_sym])
                            symbolic["computation_steps"] = steps
                        except Exception:
                            pass  
                        ...
                        for k, v in fr.items():
                            try:
                                ...
                                symbolic[k] = str(v)
                                symbolic[f"{k}_latex_warning"] = f"LaTeX conversion failed: {str(latex_err)}"
                    except Exception as e:
    ...
    if mode == "surface":
        try:
            ...
            if not surface:
                return JsonResponse({"ok": False, "error": "Surface type not specified"}, status=400)
            ...
            try:
                ...
                error_msg = str(e) if str(e) else f"Error getting surface expressions: {type(e).__name__}"
                return JsonResponse({"ok": False, "error": error_msg}, status=400)
            ...
            if compute_symbolic_flag and symbolic_quantity:
                ...
                if compute_symbolic_flag:
                    try:
                        ...
                        symbolic["symbolic_error"] = f"Error converting expressions: {str(e)}"
                        compute_symbolic_flag = False
                    ...
                    if compute_symbolic_flag:
                        ...
                        try:
                            for i in range(len(param_list)):
                                try:
                                    param_list[i] = parser.parse_input(str(param_list[i]), list(parameters))
                                except Exception as parse_err:
                                    symbolic["parse_error"] = f"Error parsing expression {i+1}: {str(parse_err)}"
                                    compute_symbolic_flag = False
                                    break
                ...
                if compute_symbolic_flag and symbolic_quantity == "first_form":
                    try:
                        # Add computation steps
                        try:
                            steps = functions.get_computation_steps("first_form", param_list, parameters)
                            symbolic["computation_steps"] = steps
                        except Exception:
                            pass  
                        ...
                        try:
                            ...
                            symbolic["G"] = str(G)
                            symbolic["I"] = str(I)
                            symbolic["first_form_latex_warning"] = f"LaTeX conversion failed: {str(latex_err)}"
                    except Exception as e:
                        error_msg = str(e) if str(e) else f"{type(e).__name__}"
                        symbolic["first_form_error"] = f"Error computing first fundamental form: {error_msg}"

                elif symbolic_quantity == "second_form":
                    try:
                        # Add computation steps
                        try:
                            steps = functions.get_computation_steps("second_form", param_list, parameters)
                            symbolic["computation_steps"] = steps
                        except Exception:
                            pass  
                        ...
                        try:
                            ...
                            symbolic["N"] = str(N)
                            symbolic["II"] = str(II)
                            symbolic["second_form_latex_warning"] = f"LaTeX conversion failed: {str(latex_err)}"
                    except Exception as e:
                        error_msg = str(e) if str(e) else f"{type(e).__name__}"
                        symbolic["second_form_error"] = f"Error computing second fundamental form: {error_msg}"
                    ...
                    try:
                        # Add computation steps
                        try:
                            steps = functions.get_computation_steps(symbolic_quantity, param_list, parameters)
                            symbolic["computation_steps"] = steps
                        except Exception:
                            pass  
                        ...
                        try:
                            if symbolic_quantity == "gaussian_curvature":
                                ...
                                try:
                                    ...
                                    symbolic["k1"] = str(k1)
                                    symbolic["k2"] = str(k2)
                                    symbolic["curvature_latex_warning"] = f"LaTeX conversion failed: {str(latex_err)}"
                        except Exception as latex_err:
                            symbolic["curvature_latex_error"] = f"LaTeX conversion failed: {str(latex_err)}"
                    except Exception as e:
                        error_msg = str(e) if str(e) else f"{type(e).__name__}"
                        
                        if "degenerate" in error_msg.lower() or "singular" in error_msg.lower():
                    ...
                    try:
                        # Add computation steps
                        try:
                            steps = functions.get_computation_steps("christoffel", param_list, parameters)
                            symbolic["computation_steps"] = steps
                        except Exception:
                            pass 
                        ...
                        for name, expr in gamma.items():
                            try:
                                ...
                                symbolic[f"{name}_latex_warning"] = f"LaTeX conversion failed: {str(latex_err)}"
                    except Exception as e:
                        error_msg = str(e) if str(e) else f"{type(e).__name__}"
                        symbolic["christoffel_error"] = f"Error computing Christoffel symbols: {error_msg}"

                elif symbolic_quantity == "gauss_equations":
                    try:
                        # Add computation steps
                        try:
                            steps = functions.get_computation_steps("gauss_equations", param_list, parameters)
                            symbolic["computation_steps"] = steps
                        except Exception:
                            pass  
                        ...
                        for i, expr in enumerate(g_eqs, start=1):
                            try:
                                ...
                                symbolic[f"Gauss_eq_{i}_latex_warning"] = f"LaTeX conversion failed: {str(latex_err)}"
                    except Exception as e:
                        error_msg = str(e) if str(e) else f"{type(e).__name__}"
                        symbolic["gauss_eq_error"] = f"Error computing Gauss equations: {error_msg}"

                elif symbolic_quantity == "codazzi_equations":
                    try:
                        # Add computation steps
                        try:
                            steps = functions.get_computation_steps("codazzi_equations", param_list, parameters)
                            symbolic["computation_steps"] = steps
                        except Exception:
                            pass  
                        ...
                        for i, expr in enumerate(c_eqs, start=1):
                            try:
                                ...
                                symbolic[f"Codazzi_eq_{i}_latex_warning"] = f"LaTeX conversion failed: {str(latex_err)}"
                    except Exception as e:
                        error_msg = str(e) if str(e) else f"{type(e).__name__}"
