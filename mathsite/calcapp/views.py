...
def compute(request):
    ...
    if mode == "curve":
        try:
            ...
            t, R = utils.numeric_curve_positions(curve, params, t0, t1, n)
            curvature, torsion = utils.curvature_torsion_from_R(R, t)
            arclen = utils.arc_length_numeric(R, t)

            symbolic = {}

            if symbolic_quantity:
                t_sym = sp.symbols("t", real=True)
                r_matrix = utils.symbolic_formula_for(curve, params)
                ...
                if symbolic_quantity == "arc_length":
                    try:
                        s_expr = utils.compute_arc_length(param_list, t_sym, bounds)
                    ...
                    try:
                        rep = utils.compute_arc_length_reparametrization(
                    ...
                    try:
                        fr = utils.compute_frenet_serret_apparatus(param_list, t_sym)
                        for k, v in fr.items():
    ...
    if mode == "surface":
        try:
            ...
            exprs_num = utils.get_surface_expressions(surface, params)
            U, V, X, Y, Z = utils.mesh_from_parametric_surfaces(
            ...
            if compute_symbolic_flag and symbolic_quantity:
                ...
                if symbolic_quantity == "first_form":
                    try:
                        I = utils.compute_first_fundamental_form(param_list, parameters)
                        E = I[0, 0]
                        F = I[0, 1]
                        G = I[1, 1]
                    ...
                    try:
                        II = utils.compute_second_fundamental_form(param_list, parameters)
                        L = II[0, 0]
                        M = II[0, 1]
                        N = II[1, 1]
                ...
                elif symbolic_quantity in ("gaussian_curvature", "mean_curvature", "principal_curvatures"):
                    try:
                        K_expr = utils.compute_gaussian_curvature(param_list, parameters)
                        H_expr = utils.compute_mean_curvature(param_list, parameters)
                    ...
                    try:
                        gamma = utils.compute_christoffel_symbols(param_list, parameters)
                    ...
                    try:
                        g_eqs = utils.compute_gauss_equations(param_list, parameters)
                    ...
                    try:
                        c_eqs = utils.compute_codazzi_equations(param_list, parameters)
